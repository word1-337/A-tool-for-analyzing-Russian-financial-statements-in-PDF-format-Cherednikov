...
- `final_pdf_analyzer.py` — backend: парсинг MinerU, расчёт коэффициентов, генерация отчёта `.txt`.
- `run_app.py` — точка входа (запускает Streamlit через Python).
...
После установки запустите `run_agent.bat` **двойным кликом**.

Браузер откроется автоматически по адресу `http://localhost:8501`.

Либо вручную из командной строки (при активированном `.venv`):
...
| Ошибка `ModuleNotFoundError: No module named 'final_pdf_analyzer'` | Запускайте `run_agent.bat` из папки проекта, а не из другого места |
